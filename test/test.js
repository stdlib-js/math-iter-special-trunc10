...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterTrunc10, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterTrunc10( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which rounds each iterated value to the nearest power of 10 toward zero', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterTrunc10( linspace( -200.0, 200.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( -200.0, 200.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = trunc10( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which rounds each iterated value to the nearest power of 10 toward zero (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	values = [ 0.0, -10.5, 11.0, -1.5 ];
	expected = [
		{
			'value': trunc10( values[ 0 ] ),
			'done': false
		},
		{
			'value': trunc10( values[ 1 ] ),
			'done': false
		},
		{
			'value': trunc10( values[ 2 ] ),
			'done': false
		},
		{
			'value': trunc10( values[ 3 ] ),
			'done': true
		}
	];
